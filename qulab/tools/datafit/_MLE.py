...
class MLE(object):
    def __init__(self):
        # qubit 位数
        self._N = 1
        # 系统各本征态的测量输出'_F=Fr + 1j*Fi'，自定义一个基准使所有测量的实部和虚部都大于0，
        # 格式为：
        # [[*, *, *, *, ...], 
        #  [*, *, *, *, ...],]
        # 分别对应实部和虚部，每一组对应一个本征态结果
        # 若只有实部，则虚部省略或者填0
        self._F = None
        # 系统经过各预操作之后的测量结果，格式和self._F一样
        self._K = None
        self._T = None
    ...
    def set_density_maritx(self):
        # 通过矩阵T生成密度矩阵
        self.set_T()
        self.density_M = np.dot(self._T.conj().T, self._T)
    ...
    def set_eigen_state_meas_result(self, F):
        ## 输入参数F为shape=[2, 2**self._N]或者shape=[1,2**self._N]的矩阵
        # 系统各本征态的测量输出'F=Fr + 1j*Fi'，自定义一个基准使所有测量的实部和虚部都大于0，
        # 格式为：
        # [[*, *, *, *, ...], 
        #  [*, *, *, *, ...],]
        # 分别对应实部和虚部，每一组对应一个本征态结果
    ...
    def set_likelihood_func(self, meas_pre_operation, meas_data):
        ...
        for op_idx in range(len(meas_pre_operation[0])):
            ...
            # 若测量结果包含实部和虚部两部，则对两部分都进行累加
            for f_idx in range(len(meas_data)):
                temp = np.dot(U, self.density_M)
                temp = np.dot(temp, U.conj().T)
                prob = np.dot(self._F[f_idx], np.diag(temp))
                self.likelihood_func += meas_data[f_idx][op_idx]*log(prob)
